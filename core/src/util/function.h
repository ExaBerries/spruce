...
namespace spruce {
	namespace util {
		...
		constexpr auto execute_impl(const FUNCTION& function, TUPLE&& args, std::index_sequence<INDEXES...> /*unused*/) noexcept {
			return function(std::get<INDEXES>(std::forward<TUPLE>(args))...);
		}

		template <typename FUNCTION, typename TUPLE>
		constexpr auto execute(const FUNCTION& function, TUPLE&& args) noexcept {
