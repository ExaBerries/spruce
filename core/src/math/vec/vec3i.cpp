...
namespace spruce {
	...
	float vec3i::mag2() const noexcept {
		return static_cast<float>(x * x + y * y + z * z);
	}

	float vec3i::mag() const noexcept {
		return std::sqrt(mag2());
	}

	float vec3i::dst2(const vec3i& vector) const noexcept {
		int32 dx = vector.x - x;
		int32 dy = vector.y - y;
		int32 dz = vector.z - z;
	...
	float vec3i::dst(const vec3i& vector) const noexcept {
		return sqrt(dst2(vector));
	}

	float vec3i::dot(const vec3i& vector) const noexcept {
		return static_cast<float>(vector.x * x + vector.y * y + vector.z * z);
	}

	vec3i& vec3i::add(const vec3i& vector) noexcept {
		x += vector.x;
		y += vector.y;
		z += vector.z;
		return *this;
	...
	vec3i& vec3i::sub(const vec3i& vector) noexcept {
		x -= vector.x;
		y -= vector.y;
		z -= vector.z;
		return *this;
	...
	vec3i& vec3i::add(int32 value) noexcept {
		x += value;
		y += value;
		z += value;
		return *this;
	...
	vec3i& vec3i::sub(int32 value) noexcept {
		x -= value;
		y -= value;
		z -= value;
		return *this;
	...
	vec3i& vec3i::scl(int32 value) noexcept {
		x *= value;
		y *= value;
		z *= value;
		return *this;
	...
	vec3i& vec3i::div(int32 value) noexcept {
		x /= value;
		y /= value;
		z /= value;
		return *this;
	...
	bool vec3i::operator==(const vec3i& vector) const noexcept {
	...
	vec3i& vec3i::operator+=(const vec3i& vector) noexcept {
		x += vector.x;
		y += vector.y;
		z += vector.z;
		return *this;
	...
	vec3i& vec3i::operator+=(int32 value) noexcept {
		x += value;
		y += value;
		z += value;
		return *this;
